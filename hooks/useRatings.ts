      ...
      .then(({ data, error }) => {
        ...
        data.forEach((row) => {
          map[row.event_id] = {
            ...
            overall: row.overall,
            energyArc: row.energy_arc ?? undefined,
            selectionStyle: row.selection_style ?? undefined,
            mixQuality: row.mix_quality ?? undefined,
            crowdSync: row.crowd_sync ?? undefined,
            tags: row.tags ?? [],
      ...
      const { error } = await supabase.from('ratings').upsert(
        {
          ...
          energy_arc: rating.energyArc ?? null,
